...
class RestApi{
  static Future<CustomerSystems>  getSystemsBasedOnUserId(var body)async{
       final response = await http.post(Uri.parse(ApiEndPoints.customerSystems),headers: {
  ...
  Future getSensorsBasedOnSystemId(var systemId)async{
    final response = await http.get(Uri.parse('https://jsonplaceholder.typicode.com/albums/1'),headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
